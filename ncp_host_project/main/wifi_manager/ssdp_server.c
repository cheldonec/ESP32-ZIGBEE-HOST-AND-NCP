...
static void send_notify(bool alive, bool update) {
    ...
    if (len > 0 && len < sizeof(buffer)) {
        ...
        ESP_LOGI(TAG, "SSDP: Sent notify (%s) ssdpip (%s)", current_ip, action);
    } else {
...
{
    // === 1. Попробуем остановить, если уже работает ===
    if (s_running) {
        ESP_LOGW(TAG, "SSDP server already running → stopping first...");
        //stop_ssdp_server();
        vTaskDelay(pdMS_TO_TICKS(20));  // Даём время на полную остановку
    }

    // === 3. Инициализация состояния ===
    s_running = true;

    // Загружаем boot_id (увеличиваем при каждом перезапуске)
    init_ssdp_load_boot_id();

    // Генерируем UUID один раз
    ...
    if (ret != pdPASS) {
        ...
        ESP_LOGI(TAG, "SSDP server task started, handle: 0x%x", (uint32_t)s_task_handle);
    }
}
...
void stop_ssdp_server(void) {
    ...
    // Удаляем задачу только если она существует
    if (s_task_handle != NULL) {
        ESP_LOGI(TAG, "Stopping SSDP server task...");
        vTaskDelay(pdMS_TO_TICKS(10));  // Даем время на завершение отправки
        vTaskDelete(s_task_handle);
        s_task_handle = NULL;
        ESP_LOGI(TAG, "SSDP server task stopped and deleted");
    } else {
        ESP_LOGW(TAG, "SSDP server task handle is NULL, nothing to delete");
    }
    s_running = false;
}
