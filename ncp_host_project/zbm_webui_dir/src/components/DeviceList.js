    ...
    <div className="w-80 bg-gray-900/40 border-r border-gray-700/50 flex-shrink-0 flex flex-col backdrop-blur-sm">
      ...
      <nav className="flex-1 overflow-y-auto">
        {devices.length === 0 ? (
          ...
          devices.map((dev) => (
            <div key={dev.ieee} className="device-item group">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3">
                    <span className="text-lg">{dev.online ? '🟢' : '🔴'}</span>
                    <div>
                      <div className="font-semibold text-white text-sm">{dev.friendly_name}</div>
                      <div className="text-xs text-gray-400 mt-0.5">
                        0x{dev.short.toString(16).toUpperCase()} • LQI: {dev.linkquality || '?'}
                      </div>
                    </div>
                  </div>
                </div>
