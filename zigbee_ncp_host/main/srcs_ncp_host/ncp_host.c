...
static const char *TAG = "ncp_host.c";
...
static esp_err_t esp_host_process_event(esp_host_dev_t *dev, esp_host_ctx_t *ctx)
{
    ...
    if (!bus || !bus->input_buf || !bus->output_buf || !bus->read || !bus->write) {
        ...
        if (!bus->read) {
            ESP_LOGE(TAG, "Read function is NULL");
        }
        if (!bus->write) {
            ESP_LOGE(TAG, "Write function is NULL");
        }
        return ESP_FAIL;
    }
...
{
    ESP_LOGI(TAG, "start esp_host_main_task(void *pv)");
    esp_host_dev_t *dev = (esp_host_dev_t *)pv;
    esp_host_ctx_t host_ctx;

    dev->run = true;

    ESP_LOGI(TAG, "start esp_host_bus_start command from esp_host_main_task");
    esp_host_bus_start(dev->bus);

    ESP_LOGI(TAG, "start dev->queue = xQueueCreate from esp_host_main_task");
    // ✅ Проверяем и чистим очередь
    /*if (dev->queue) {
        ESP_LOGW(TAG, "dev->queue already exists! Deleting...");
        vQueueDelete(dev->queue);
    }*/
    dev->queue = xQueueCreate(HOST_EVENT_QUEUE_LEN, sizeof(esp_host_ctx_t));
    
    ESP_LOGI(TAG, "esp_host_main_task STARTED and working while (dev->run)  ");
    while (dev->run) {
...
esp_err_t esp_host_init(esp_host_connection_mode_t mode)
{
    ...
    ESP_LOGI(TAG, "start esp_host_init -> esp_host_bus_init()");
...
esp_err_t esp_host_deinit(void)
{
    ...
    if (s_host_dev.queue) {
        ESP_LOGI(TAG, "start esp_host_deinit -> vQueueDelete(s_host_dev.queue);");
...
{
    ESP_LOGI(TAG, "start esp_host_start");
    if (s_host_dev.run) {
        return ESP_ERR_INVALID_ARG;
    }

    return (xTaskCreate(esp_host_main_task, "host_task", HOST_TASK_STACK, &s_host_dev, HOST_TASK_PRIORITY, NULL) == pdTRUE) ? ESP_OK : ESP_FAIL;
}
