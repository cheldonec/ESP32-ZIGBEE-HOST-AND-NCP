#ifndef NCP_HOST_ZB_API_ZDO_H   
#define NCP_HOST_ZB_API_ZDO_H
#include <math.h>
#include <stdint.h>
#include "stdbool.h"
#include "esp_err.h"
...
#define LOCAL_ESP_ZB_PACKED_STRUCT __attribute__ ((packed))
...
/*****************************************************************************************************************************
 * @brief Структура ответа на zb_manager_zdo_active_ep_req для передачи в event (HOST <- NCP) ZB_MANAGER_ACTIVE_EP_RESP 0x5558
 */
typedef struct zb_manager_active_ep_resp_message_s {
        local_esp_zb_zdp_status_t   status;
        uint8_t                     ep_count; 
...
} local_esp_zb_zdo_active_ep_req_param_t;
...
/*****************************************************************************************************************************
 * @brief Функция запроса активных эндпоинтов (HOST -> NCP) ZB_MANAGER_ACTIVE_EP_REQ_CMD 0x5557
 */
esp_err_t zb_manager_zdo_active_ep_req(local_esp_zb_zdo_active_ep_req_param_t *cmd_req, local_esp_zb_zdo_active_ep_callback_t user_cb, void *user_ctx);
...
} LOCAL_ESP_ZB_PACKED_STRUCT local_esp_zb_af_simple_desc_1_1_t;
...
/*****************************************************************************************************************************
 * @brief Структура ответа на zb_manager_zdo_simple_desc_req для передачи в event (HOST <- NCP) ZB_MANAGER_SIMPLE_DESC_RESP 0x5556
 */
typedef struct {
    local_esp_zb_zdp_status_t status;
    local_esp_zb_af_simple_desc_1_1_t* simple_desc;
    local_esp_zb_zdo_simple_desc_callback_t user_cb;                                   /*!< A ZDO match desc request callback */
    void* user_ctx; // Сохраняем контекст, чтобы использовать в обработчике
} zb_manager_simple_desc_resp_message_t;

/*****************************************************************************************************************************
 * @brief Очищает массив ep внутри zb_manager_simple_desc_resp_message_t после передачи в event, сам объект не удаляется, так как 
 * после обработки в event он будет удалён автоматически
 */
...
} local_esp_zb_zdo_simple_desc_req_param_t;
...
/*****************************************************************************************************************************
 * @brief Функция запроса SIMPLE_DESC (HOST -> NCP) ZB_MANAGER_SIMPLE_DESC_REQ_CMD 0x5555
 */
...
/** 
 * @brief Структура для хранения параметров запроса на simple_desc во время сопряжения (устройство биндит координатор) через event_post
*/
typedef struct {
    uint16_t                                short_addr;
...
/*****************************************************************************************************************************
 * @brief Структура ответа на esp_zb_zdo_device_bind_req для передачи в event (HOST <- NCP) ESP_NCP_ZDO_BIND_SET 0x0200
 */
typedef struct {
    local_esp_zb_zdp_status_t status;
    void* user_ctx;
...
/*****************************************************************************************************************************
 * @brief Очищает данные внутри zb_manager_bind_resp_message_t после передачи в event, сам объект не удаляется, так как 
 * после обработки в event он будет удалён автоматически
 */
...
} local_esp_zb_zdo_bind_req_param_t;


 /*****************************************************************************************************************************
* @brief Bind request callback
 *
 */
typedef void (*local_esp_zb_zdo_bind_callback_t)(local_esp_zb_zdp_status_t zdo_status, void *user_ctx);


 /*****************************************************************************************************************************
 * @brief Функция запроса BIND_REQ (HOST -> NCP) ESP_NCP_ZDO_BIND_SET 0x0200
 */
...
/** 
 * @brief Структура для хранения параметров запроса на bind во время сопряжения (устройство биндит координатор) через event_post
...
/**
 * @brief Структура для биндинга между устройствами
 */
typedef struct {
    uint16_t src_short_addr; // кто биндит input cluster
    uint8_t src_endpoint;
    uint16_t dst_short_addr; // кого биндят output cluster
    uint8_t dst_endpoint;
    uint16_t cluster_id;
} delayed_bind_dev_dev_req_t; // src bind -> dst
...
//================================================================================================================================
 ...
 /*****************************************************************************************************************************
 * @brief Полный Node Descriptor (копия из esp_zb_zcl_common.h)
 */
typedef struct {
...
} zb_manager_node_desc_t;


 /*****************************************************************************************************************************
 * @brief Структура ответа на node_desc_resp для передачи в event (HOST <- NCP) ZB_MANAGER_NODE_DESC_RSP 0x555A
 */
typedef struct zb_manager_node_desc_resp_message_s{
    uint8_t status;
    uint16_t short_addr;
...
void zb_manager_free_node_desc_resp(zb_manager_node_desc_resp_message_t* resp);
 ...
 /*****************************************************************************************************************************
 * @brief Функция запроса NODE_DESC_REQ (HOST -> NCP) ZB_MANAGER_NODE_DESC_REQ 0x5559
 */
...
} local_esp_zb_zdo_match_desc_req_param_t;

/*****************************************************************************************************************************
 *
 * @brief A ZDO match desc request callback for user to get response info.
 */
typedef void (*local_esp_zb_zdo_match_desc_callback_t)(local_esp_zb_zdp_status_t zdo_status, uint16_t addr, uint8_t endpoint, void *user_ctx);
