...
esp_err_t zbm_dev_base_dev_update_from_read_response_old(device_custom_t* dev, endpoint_custom_t* ep, zb_manager_cmd_read_attr_resp_message_t* read_resp)
{
    esp_err_t result = ESP_FAIL;
    ...
    esp_err_t result = ESP_FAIL;
            ...
            if (rep->cluster == ESP_ZB_ZCL_CLUSTER_ID_BASIC) {
                ...
                if (dev->server_BasicClusterObj) {
                        ...
                        zb_manager_basic_cluster_update_attribute(dev->server_BasicClusterObj, rep->attr.attr_id, rep->attr.attr_type, value, rep->attr.attr_len);      
                }
            }
            // 🔹 Power Configuration (0x0001)
                ...
                if (dev->server_PowerConfigurationClusterObj) {
                    void *value = rep->attr.attr_value;
                    zb_manager_power_config_cluster_update_attribute(dev->server_PowerConfigurationClusterObj, rep->attr.attr_id,rep->attr.attr_type, value, rep->attr.attr_len);
            ...
            // 🔹 Cluster на уровне endpoint'а
                ...
                if (rep->cluster == ESP_ZB_ZCL_CLUSTER_ID_TEMP_MEASUREMENT) {
                    ...
                    if (ep->server_TemperatureMeasurementClusterObj) {
                        void *value = rep->attr.attr_value;
                        zb_manager_temp_meas_cluster_update_attribute(ep->server_TemperatureMeasurementClusterObj, rep->attr.attr_id,rep->attr.attr_type, value, rep->attr.attr_len);
                        //ws_notify_device_update_unlocked(dev);
                ...
                // 🔹 Humidity (0x0405)
                    ...
                    if (ep->server_HumidityMeasurementClusterObj) {
                        void *value = rep->attr.attr_value;
                        zb_manager_humidity_meas_cluster_update_attribute(ep->server_HumidityMeasurementClusterObj, rep->attr.attr_id,rep->attr.attr_type, value, rep->attr.attr_len);
                        //ws_notify_device_update_unlocked(dev);
                            /*zb_rule_trigger_state_update(
                                rep->src_address.u.short_addr,
                                rep->cluster,
                ...
                // 🔹 On/Off (0x0006)
                    ...
                    if (ep->server_OnOffClusterObj) {
                            void *value = rep->attr.attr_value;
                            zb_manager_on_off_cluster_update_attribute(ep->server_OnOffClusterObj, rep->attr.attr_id,rep->attr.attr_type, value, rep->attr.attr_len);
                            //ws_notify_device_update_unlocked(dev);
                ...
                // 🔹 Неизвестный кластер
                    ...
                    if (!custom_cluster) {
                        ...
                        ESP_LOGI(TAG, "🆕 Created custom cluster 0x%04x for device 0x%04x", rep->cluster, dev->short_addr);
                    }
                    ...
                    // Обновляем значение
                    if (custom_attr) {
                        zbm_update_custom_attribute_value(custom_attr, (const uint8_t*)rep->attr.attr_value, rep->attr.attr_len);
